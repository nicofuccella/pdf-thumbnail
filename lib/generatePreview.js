...
const toJpg = (body) => () => new Promise((resolve, reject) => {
...
module.exports = (body, options = {}) => {
  const promises = Object.entries(options).reduce(([name, operation], accumulator) => [
    ...accumulator,
    (data) => allTasks[name](data, operation)
  ], [toJpg(body)])
  ...
  return promiseWaterfall([ ...promises, gmToBuffer ])
}
