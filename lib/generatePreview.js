...
module.exports = (body, options = {}) => new Promise((resolve, reject) => {
  const tasks = [() => gm(body, 'test.pdf[0]')
    .setFormat('jpeg')];
  // Non so come metterla nella waterfall perche' e' una promise e gli devo passare anche le options
  //if (options.crop) tasks.push(allTasks['crop']);
  if (options.resize) tasks.push((data) => allTasks.resize(data, options.resize));
  if (options.compress) tasks.push((data) => allTasks.compress(data, options.compress));
  promiseWaterfall(tasks)
    .then(data => gmToBuffer(data).then(buffer => resolve(buffer)))
